...
async def version() -> dict[str, Any]:
    """Agent version + manifest schema. PUBLIC - bypasses auth."""
    return {
...
async def reboot(body: dict | None = None) -> dict[str, Any]:
    ...
    and seren-setup.sh's install_agent_common. Existing installs need the
    ...
    if result.returncode != 0:
        return {
            ...
            "command": shlex.join(cmd),
            "hint": (
                "If stderr mentions 'a password is required', the agent's "
...
async def agent_update(
    ...
    """Receive a seren-agent.tar.gz from the RuntimeHost and run the update script.
    ...
    The RuntimeHost streams the package as multipart/form-data with two parts:
        ...
        dest_path - absolute path on this node where the file should land
                    (e.g. /home/seren/seren-install). The seren-agent-update.sh
                    script must live in the same directory.

    The file is saved synchronously, then seren-agent-update.sh is launched as
    a detached background process before we return. The update script is expected
    to restart the agent - the response is sent first so the HTTP connection
    ...
    tar_path = os.path.join(dest_dir, "seren-agent.tar.gz")
    script_path = os.path.join(dest_dir, "seren-agent-update.sh")
    ...
    # Launch the update script detached from the agent's process group.
    #
    # seren-agent.service uses KillMode=process (see common.sh), which means
    ...
    # its own session and survives the agent being stopped, then brings it
