"""Tests for seren_observatory.config.load_config().
...
from seren_observatory.config import ObservatoryConfig, load_config
...
def test_bearer_token_in_yaml_is_ignored(cfg_path, capsys):
    """THE load-bearing test: a token in the yaml must NOT become the observatory's
...
def test_seren_observatory_env_alias(cfg_path, monkeypatch):
    cfg_path.write_text("server:\n  port: 9999\n")
