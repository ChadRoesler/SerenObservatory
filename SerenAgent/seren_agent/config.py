...
who set up one service already knows how to set up this one:
    ...
    * config resolves: --config  ->  $SEREN_AGENT_CONFIG  ->
      ~/seren-agent/seren-agent.yaml  ->  built-in defaults
    * the file is named seren-agent.yaml

DELIBERATE EXCEPTION - the bearer token is NOT here. Unlike SerenMemory, the
agent's token is a SAFETY INTERLOCK, not a config knob: this plane can restart
...
by auth.load_token(). The agent fails CLOSED on mutating methods when no token
...
class AgentConfig(BaseModel):
    """seren-agent network config. Defaults match the cluster convention:
    bind all interfaces (trusted LAN) on 7777.

    NOTE: no token field here, on purpose - see module docstring.
    """

    # Bind all interfaces by default - the agent is a cluster plane meant to be
...
_DEFAULT_CONFIG_PATH = Path.home() / "seren-agent" / "seren-agent.yaml"


def _resolve_config_path(explicit_path: Optional[str] = None) -> Optional[Path]:
    """--config -> $SEREN_AGENT_CONFIG -> ~/seren-agent/seren-agent.yaml -> None."""
    if explicit_path:
...
def _load_yaml_lenient(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {}
    if not _HAS_YAML:
        print(f"[seren-agent] config: pyyaml not installed; ignoring {path}")
        return {}
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
    except Exception as e:
        print(f"[seren-agent] config: failed to parse {path}: {e} (using defaults)")
        return {}
    if data is None:
        return {}
    if not isinstance(data, dict):
        print(f"[seren-agent] config: {path} top-level must be a mapping; got "
              f"{type(data).__name__} (using defaults)")
        return {}
    return data


def _apply_server_overrides(cfg: AgentConfig, server: dict[str, Any], *, source: str) -> None:
    ...
    for key, raw in server.items():
        if key == "bearer_token":
            ...
            print("[seren-agent] config: 'bearer_token' in the yaml is ignored "
                  "by design - the agent token lives in ~/.seren/secrets.json "
                  "(run seren-secrets.sh). See config.py for why.")
            continue
        if key not in known:
            print(f"[seren-agent] config: ignoring unknown server key '{key}' from {source}")
            continue
        try:
            current = cfg.model_dump()
            current[key] = raw
            cfg.__dict__.update(AgentConfig.model_validate(current).__dict__)
        except Exception as e:
            print(f"[seren-agent] config: ignored bad value for '{key}' from {source}: {e}")
...
def load_config(path: Optional[str] = None) -> AgentConfig:
    ...
    if yaml_path is not None:
        ...
        if isinstance(server, dict):
            ...
            print(f"[seren-agent] config: 'server' in {yaml_path} must be a mapping; ignoring")
