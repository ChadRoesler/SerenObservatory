...
catches it) AND a rotating file at ~/seren-logs/agent-requests.log so you
...
Why log to a file the user owns:
  - 'sudo journalctl -u seren-agent' requires a password by default
...
def _setup_logger() -> logging.Logger:
    ...
    Sets up two handlers:
      ...
      2. RotatingFileHandler → ~/seren-logs/agent-requests.log
    ...
    """
    logger = logging.getLogger("seren-agent.requests")
    if logger.handlers:
    ...
    # File → ~/seren-logs/agent-requests.log, daily rotation, 7 days kept
    log_dir = Path.home() / "seren-logs"
    try:
        log_dir.mkdir(parents=True, exist_ok=True)
        file_handler = logging.handlers.TimedRotatingFileHandler(
            filename=log_dir / "agent-requests.log",
            when="midnight",
            backupCount=7,
            encoding="utf-8",
        )
        ...
        # Don't crash the agent if the log dir isn't writable - fall back
