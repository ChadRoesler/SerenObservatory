...
Each module under `observatory.services.<name>` exposes a `register(router, name)`
...
The observatory's main app calls these per-service register functions only for
...
# a non-Jetson host, so the observatory no longer manages a chroma service. See
