...
def _systemd_unit_name(manifest: dict[str, Any]) -> str | None:
    ...
    The observatory's pre-Path-C self-manifest puts it nested at
...
def _docker_run(args: list[str], timeout: float = 30.0) -> dict[str, Any]:
    ...
    We use sudo because the observatory may not have live docker group membership
...
def _docker_container_state(manifest: dict[str, Any]) -> dict[str, Any] | None:
    ...
    to a live shellout if the cache is cold (first call after observatory
